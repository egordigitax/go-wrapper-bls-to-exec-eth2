...
```sh
docker run -it --rm -v $(pwd)/validator_keys:/app/validator_keys ethereum/wrapper
```

You can also run the tool with optional arguments:

```sh
docker run -it --rm -v $(pwd)/validator_keys:/app/validator_keys ethereum/wrapper new-mnemonic --num_validators=<NUM_VALIDATORS> --mnemonic_language=english --folder=<YOUR_FOLDER_PATH>
```

Example for 1 validator on the [Prater testnet](https://prater.launchpad.ethereum.org/) using english:

```sh
docker run -it --rm -v $(pwd)/validator_keys:/app/validator_keys ethereum/wrapper new-mnemonic --num_validators=1 --mnemonic_language=english --chain=prater
```
